...
def main(graph): 
	...
	for e in graph.getEdges():
		...
		if cS == cT:
			...
			cPair.sort()
			cPair = ", ".join(cPair)
			if cPair not in diffToE:
				diffToE[cPair] = []
			diffToE[cPair].append(e)
						
	print "same country ends: ",countSame
	print "different country ends: ",countDiff
	
	print "countries to same ends"
	print {c:len(sameToE[c]) for c in sameToE}
	
	print "countries that interact"
	print {c:len(diffToE[c]) for c in diffToE if len(diffToE) > 1}
